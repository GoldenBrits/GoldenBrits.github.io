...
function Colour()
{
  ...
  //splits the time into different section so it can be used
  var mins = Time.getMinutes();
  var hrs = Time.getHours();
  var CurrentDate = Time.getDate();
  var month = Time.getMonth();
  
  //Uses the mins variable to set the hue of the background image
  var Hue = 6*mins;
  //Uses the hrs and mins (My maths Teacher helped me with the Maths behind the brightness)
  var Brightness = 40*Math.pow(Math.sin(hrs*3+mins), 2)+30;
  
  console.log(Hue, Brightness);
  
  //sets the css variables Hue and Brightness to the values of the JS Variables Hue and Brightness
